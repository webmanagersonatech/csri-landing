...
import { FaInfoCircle, FaUsers, FaStar, FaEye, FaBullseye, FaClipboardList, FaUserShield, FaFacebookF, FaTwitter, FaInstagram, FaLinkedinIn } from "react-icons/fa";
type Profile = {
  id: number;
  name: string;
  role: string;
...
const profiles = [{
  ...
  image: "/images/team/valliappa-chairman-sona-gro.webp",
...
Guided by his belief that <strong>industry and society thrive together,</strong> C. Valliappa continues to inspire transformative growth — building businesses of global relevance while nurturing sustainable communities.`
  ...
  image: "/images/team/chocko-valliappa.webp",
...
Through these measures, Chocko Valliappa has positioned himself not only as a technology and business leader but also as a champion of inclusive growth, ensuring that innovation benefits the commons and the deprived.`
  ...
  role: "Vice Chairman – The Sona Institutions",
  image: "/images/team/thyagu-valliappa.webp",
...
`
  ...
  role: "Principal",
  image: "/images/team/senthil-kumar.webp",
...
const AboutUssection = () => {
  ...
  return <>
      <ReuseBanner image="/images/banner/about-us.webp" title={`About Us${activeTabData && activeTabData.label !== "About Us" ? ` - ${activeTabData.label}` : ""}`} subtitle="The Vision and Values of the Sona Group" />

      <Breadcrumb items={[{
      label: "Home",
      href: "/"
    }, {
      label: activeTabData ? activeTabData.label : "About Us",
      href: "/about"
    }]} />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <div className="flex flex-col lg:flex-row gap-10">
            ...
            <div className="flex-1 lg:w-3/4">
                ...
                {activeTab === "about" ? <div key="about">
                    {aboutSections.map((section, idx) => <div key={idx} className={`flex flex-col md:flex-row items-stretch gap-10  ${idx % 2 === 1 ? "md:flex-row-reverse" : ""}`}>
                    ...
                    __html: section.content
                        ...
                        <div className="relative flex-1 h-full min-h-[280px]">
                          <Image src={section.image} alt={`about-${idx}`} fill sizes="(max-width: 768px) 100vw, 50vw" className="shadow-xl object-cover" />
                        </div>
                      </div>)}
                  </div> : activeTab === "community" ? <div key="community" className="space-y-2">
                    {communitySections.map((section, idx) => <div key={idx} className={`flex flex-col md:flex-row items-stretch gap-8  transition-shadow duration-300 ${idx % 2 === 1 ? "md:flex-row-reverse" : ""}`}>

                        {/* IMAGE BLOCK */}
                        <div className="relative flex-1 w-full md:w-1/2 h-full min-h-[260px] md:min-h-[320px] overflow-hidden">
                          <Image src={section.image} alt={`community-${idx}`} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover transform hover:scale-105 transition-transform duration-500" />
                        </div>

                        {/* TEXT BLOCK */}
                        <div className="flex-1 w-full md:w-1/2 h-full flex items-center text-gray-700">
                          <p className="leading-relaxed text-justify" dangerouslySetInnerHTML={{
                    __html: section.content
                  }}></p>
                        </div>
                  ...
                  </div> : activeTab === "leadership" ? <div key="leadership" className="space-y-2">
                    <section className="mb-16">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 ">
                        {profiles.map((profile: any) => <header key={profile.id} onClick={() => setSelectedProfile(profile)} className="cursor-pointer px-2 py-6 mt-12 flex flex-col justify-center items-center text-center border border-gray-300 dark:border-gray-700  shadow-md bg-white dark:bg-gray-800 transition hover:shadow-xl">
                            <Image className="inline-flex object-cover border-4 border-blue-600 rounded-full shadow-[5px_5px_0_0_rgba(0,0,0,1)] shadow-blue-600/100 bg-blue-50 text-blue-600 !h-48 !w-48" src={profile.image} alt={profile.name} width={192} height={192} />
                            ...
                            <div className="flex space-x-4 mt-3 text-gray-500 justify-center">
                              {profile.facebook ? <a href={profile.facebook} target="_blank" rel="noopener noreferrer" aria-label="Facebook">
                                  <FaFacebookF className="hover:text-blue-600 transition-colors" />
                                </a> : null}

                              {profile.twitter ? <a href={profile.twitter} target="_blank" rel="noopener noreferrer" aria-label="Twitter">
                                  <FaTwitter className="hover:text-blue-400 transition-colors" />
                                </a> : null}


                              {profile.instagram ? <a href={profile.instagram} target="_blank" rel="noopener noreferrer" aria-label="Instagram">
                                  <FaInstagram className="hover:text-pink-500 transition-colors" />
                                </a> : null}

                              {profile.linkedin ? <a href={profile.linkedin} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn">
                  ...
                  </div> : Object.keys(tabContents).filter(key => key === activeTab).map(key => <div key={key} className="flex flex-col md:flex-row items-stretch gap-10 my-4 min-h-[380px]">
                        {/* LEFT: CONTENT */}
                        <div className="flex-1 leading-relaxed text-justify h-full">
                          <ul className="space-y-3">
                            {tabContents[key].content.split("\n").filter((item: any) => item.trim() !== "").map((item: any, idx: any) => <li key={idx} className="flex items-start gap-3">
                        ...
                        {/* RIGHT: IMAGE */}
                        <div className="relative flex-1 h-full min-h-[280px]">
                          <Image src={tabContents[key].image} alt={key} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
                        </div>
                      </div>)}
            ...
            <div className="lg:w-1/4 flex flex-col gap-4 self-start sticky top-20">
              {tabs.map(tab => <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 p-3 font-semibold rounded-lg transition-colors ${activeTab === tab.id ? "bg-blue-100 text-blue-600" : "text-gray-600 hover:bg-gray-200"}`}>
                  {tab.icon}
                  {tab.label}
                </button>)}
            </div>
          ...
          {selectedProfile && <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 p-4" onClick={() => setSelectedProfile(null)} role="dialog" aria-modal="true">
              <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
                <button onClick={() => setSelectedProfile(null)} className="absolute top-3 right-3 text-gray-400 hover:text-gray-700 dark:hover:text-white transition text-2xl font-bold" aria-label="Close">
                  ✕
                </button>

                {/* Profile Header */}
                <div className="text-center mb-6">
                  <Image src={selectedProfile.image} alt={selectedProfile.name} width={128} height={128} className="w-32 h-32 mx-auto rounded-full border-4 border-blue-500 mb-4" />
                ...
                {/* Biography */}
                <div className="text-gray-700 dark:text-gray-300 text-justify text-base leading-relaxed whitespace-pre-line" dangerouslySetInnerHTML={{

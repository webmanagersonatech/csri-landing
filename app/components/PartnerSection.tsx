...
import { FaUsers, FaGraduationCap, FaBuilding, FaBriefcase, FaMoneyBillWave, FaChalkboardTeacher, FaHandsHelping, FaHandshake, FaLightbulb, FaGlobe } from "react-icons/fa";
...
const partnerData: PartnerDataType[] = [{
  ...
  description: "Supporting community programs with impactful and meaningful CSR contributions.",
  imgSrc: "/images/wework/avatar.svg"
}, {
  title: "Government Agency",
  description: "Collaborating on sustainable development and wide-reaching social initiatives.",
  imgSrc: "/images/wework/avatar3.svg"
}, {
  title: "Foundation Partner",
  description: "Driving health, education, and environmental programs for lasting impact.",
  imgSrc: "/images/wework/avatar4.svg"
}, {
  title: "Local Community Network",
  description: "Connecting CSR projects directly with the local communities that benefit.",
  imgSrc: "/images/wework/avatar.svg"
}, {
  title: "Skill Development Partner",
  description: "Empowering youth and communities through structured training programs.",
  imgSrc: "/images/wework/avatar3.svg"
}];
const settings = {
  dots: false,
...
const corporatePartners = [{
  name: "Company A",
  logoSrc: "https://img.freepik.com/premium-vector/beautiful-unique-logo-design-ecommerce-retail-company_1287271-14770.jpg?uid=R139790849&ga=GA1.1.898221838.1748329700&semt=ais_hybrid&w=740&q=80" // replace with your image path or URL
}, {
  name: "Company B",
  logoSrc: "https://img.freepik.com/premium-vector/diverse-group-individuals-standing-together-top-bridge-overlooking-view-develop-clean-elegant-logo-conference-management-firm_538213-49725.jpg?uid=R139790849&ga=GA1.1.898221838.1748329700&semt=ais_hybrid&w=740&q=80"
}, {
  name: "Company C",
  logoSrc: "https://img.freepik.com/free-vector/symetrical-business-logo_1025-939.jpg?uid=R139790849&ga=GA1.1.898221838.1748329700&semt=ais_hybrid&w=740&q=80"
}, {
  name: "Company D",
  logoSrc: "https://img.freepik.com/premium-vector/coffee-community-logo-design-vector_474888-2181.jpg?uid=R139790849&ga=GA1.1.898221838.1748329700&semt=ais_hybrid&w=740&q=80"
}];
const PartnersSection = () => {
  ...
  useEffect(() => {
    if (tabQuery) {
      const tabExists = tabs.find(t => t.id.toLowerCase() === tabQuery.toLowerCase());
      if (tabExists) setActiveTab(tabExists.id);
    }
  }, [tabQuery]);
  const handleTabClick = (id: string) => setActiveTab(id);
  return <>
      <ReuseBanner image="https://img.freepik.com/premium-photo/business-handshake-partners_160672-4152.jpg" title="Our Partners" subtitle="Collaborating for a Sustainable Future" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-6 border-b border-gray-300 dark:border-gray-700">
            <div className="flex flex-wrap justify-center gap-12 px-4 md:px-0">
              {tabs.map(tab => <button key={tab.id} onClick={() => handleTabClick(tab.id)} className={`relative flex items-center gap-2 pb-2 font-semibold transition-colors ${activeTab === tab.id ? "text-blue-600 dark:text-blue-400" : "text-gray-600 dark:text-gray-300 hover:text-blue-500 dark:hover:text-blue-300"}`}>
                  {tab.icon}
                  {tab.label}
                  {activeTab === tab.id && <span className="absolute left-0 bottom-0 w-full h-1 bg-blue-600 dark:bg-blue-400 rounded-full" />}
                </button>)}
            </div>
          </div>
          ...
          <div>
            {activeTab === "whyPartner" && <div className="py-12  space-y-12">
            ...
            {activeTab === "alumni" && <div className="py-12 space-y-16 bg-gray-50 dark:bg-gray-900">
                {/* Heading */}
                ...
                description: "Support initiatives like women’s training, rural entrepreneurship, or climate awareness programs."
              }, {
                title: "Mentorship & Guidance",
                icon: <FaChalkboardTeacher className="text-blue-600 w-12 h-12 mb-4" />,
                description: "Offer your expertise to guide young changemakers and entrepreneurs."
              }, {
                title: "Corporate & CSR Collaboration",
                icon: <FaBuilding className="text-blue-600 w-12 h-12 mb-4" />,
                description: "Connect us with potential sponsors or funding opportunities to amplify impact."
              }, {
                title: "Volunteering & Advocacy",
                icon: <FaHandsHelping className="text-blue-600 w-12 h-12 mb-4" />,
                description: "Join initiatives, spread awareness, and champion social causes."
              }].map((item, idx) => <div key={idx} className="bg-white dark:bg-gray-800 p-6 shadow hover:shadow-xl transition flex flex-col items-center text-center">
                ...
                <div className="text-center mt-12">
                  ...
                  <a href="#alumni-appeal" className="inline-block px-8 py-3 bg-blue-600 text-white font-semibold rounded-xl shadow hover:bg-blue-700 transition">
                    Appeal to Alumni
                  </a>
                </div>
              </div>}

            {activeTab === "ngo" && <div className="py-16 space-y-16 bg-gray-50 dark:bg-gray-900 px-4 md:px-12">
                {/* Heading */}
                <div className="text-center max-w-3xl mx-auto">
                  <h2 className="text-4xl font-extrabold text-gray-900 dark:text-white mb-4">
                    Partner with CSRI
                ...
                description: "Co-design and implement programs that address community needs and create measurable impact."
              }, {
                title: "Innovation & Resources",
                icon: <FaLightbulb className="text-blue-600 w-12 h-12 mb-4" />,
                description: "Share knowledge, resources, and innovative solutions to scale outreach effectively."
              }, {
                title: "Global & Local Outreach",
                icon: <FaGlobe className="text-blue-600 w-12 h-12 mb-4" />,
                description: "Extend programs to underserved communities, ensuring inclusive development across regions."
              }, {
                title: "Capacity Building",
                icon: <FaUsers className="text-blue-600 w-12 h-12 mb-4" />,
                description: "Empower grassroots teams and local partners through training, mentorship, and collaborative learning."
              }].map((item, idx) => <div key={idx} className="bg-white dark:bg-gray-800 p-6 shadow hover:shadow-xl transition flex flex-col items-center text-center rounded-xl">
                      <div className="text-5xl mb-4">{item.icon}</div>
                      <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">
                        {item.title}
                      </h3>
                ...
                <div className="text-center mt-12">
                  ...
                  <a href="#ngo-partner" className="inline-block px-8 py-3 bg-blue-600 text-white font-semibold rounded-xl shadow hover:bg-blue-700 transition">
                    Join Partner with Us
                  </a>
                </div>
              </div>}

            {activeTab === "corporate" && <div className="py-16 px-4 md:px-12 space-y-16 bg-gray-50 dark:bg-gray-900">
                {/* Heading */}
                <div className="text-center max-w-3xl mx-auto">
                  <h2 className="text-4xl font-extrabold text-gray-900 dark:text-white mb-4">
                ...
                <div>
                  <Slider {...settings}>
                    {partnerData.map((partner, idx) => <div key={idx} className="px-3">
                        <div className="h-full bg-white dark:bg-gray-800 rounded-3xl shadow-xl p-6 flex flex-col items-center justify-between transition hover:shadow-2xl">
                          <Image src={partner.imgSrc} alt={partner.title} width={120} height={120} className="rounded-full mb-4" />
                ...
                <div className="flex flex-wrap justify-center items-center gap-8 mt-12">
                  {corporatePartners.map((partner, idx) => <div key={idx} className="flex items-center justify-center p-4">
                      <Image src={partner.logoSrc} alt={partner.name} width={120} height={60} className="object-contain grayscale hover:grayscale-0 transition duration-300" />
                    </div>)}
                </div>
              </div>}
          </div>

...
export default function ProgramDetails({
  ...
  return <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 pt-32">
      {/* Program Info */}
      <div className="flex flex-col lg:flex-row gap-8 items-center">
        {/* Image */}
        <div className="relative lg:w-1/2 w-full h-80 overflow-hidden shadow-lg rounded-xl">
          <Image src={program.image} alt={program.title} fill sizes="(max-width: 1024px) 100vw, 50vw" className="object-cover transform hover:scale-105 transition-transform duration-500" />
        </div>
        ...
        <div className="lg:w-1/2 w-full flex flex-col gap-4">
          ...
          <button onClick={() => setIsOpen(true)} className="relative mt-6 px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors duration-300 w-max">
            Join Program
      ...
      {/* Modal */}
        ...
        {isOpen && <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setIsOpen(false)} // optional close on backdrop click
    >
            <div className="bg-white rounded-lg p-8 w-full max-w-md shadow-lg relative" onClick={e => e.stopPropagation()} // prevents closing when clicking inside
      >

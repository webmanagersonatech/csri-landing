...
const videoData: VideoType[] = [{
  ...
  name: "Sneha M.",
  profession: "Orientation on Women Technology",
  videoId: "5RgjCFhmI5g"
}];
export default function VideoTestimonials() {
  ...
  return <section id="testimonials" className="dark:bg-gray-100 dark:text-gray-800">
      <div className="container py-12 px-4 sm:px-6 lg:px-8 mx-auto max-w-7xl">
        ...
        {/* Slider */}
        <Slider {...settings}>
          {videoData.map((item, i) => <div key={i} className="p-2 sm:p-3">
              <div className="relative rounded-lg overflow-hidden shadow-md hover:shadow-xl transition cursor-pointer" onClick={() => setOpenVideo(item.videoId)}>
                {/* Dynamic YouTube thumbnail */}
                <img src={`https://img.youtube.com/vi/${item.videoId}/hqdefault.jpg`} alt={item.name} loading="lazy" className="w-full h-40 sm:h-48 md:h-52 object-cover" />
      ...
      {/* Modal */}
      {openVideo && <div className="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center z-50 p-4 sm:p-6">
          <div className="relative w-full max-w-3xl">
            <button className="absolute top-2 right-2 text-white text-3xl sm:text-4xl z-50" onClick={() => setOpenVideo(null)}>
              ✕
            </button>
            <div className="w-full aspect-video">
              <iframe width="100%" height="100%" src={`https://www.youtube.com/embed/${openVideo}?autoplay=1`} title="YouTube video" frameBorder="0" allow="autoplay; encrypted-media" allowFullScreen />
            </div>
          </div>
        </div>}
    </section>;
